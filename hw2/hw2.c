...
int main()
{
    ...
    gettimeofday(&start, NULL);
    int_radixsort(arr,Size);
    gettimeofday(&end, NULL);
    diff = 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec; // 實際的時間差
    printf("int_radixsort int:%ld us (equal %f sec)\n", diff, diff / 1000000.0);
    sum += diff / 1000000.0;
    gettimeofday(&start, NULL);
    str_radixsort(str,Size);
    gettimeofday(&end, NULL);
    diff = 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec; // 實際的時間差
    printf("str_radixsort int:%ld us (equal %f sec)\n", diff, diff / 1000000.0);
    sum += diff / 1000000.0;
    fclose(intData);
    fclose(strData);
    printf("sum:%f\n",sum);
    ...
    gettimeofday(&start, NULL);
    int_heapSort(arr,Size);
    gettimeofday(&end, NULL);
    diff = 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec; // 實際的時間差
    printf("int_heapsort int:%ld us (equal %f sec)\n", diff, diff / 1000000.0);
    sum += diff / 1000000.0;
    gettimeofday(&start, NULL);
    str_heapSort(str,Size);
    gettimeofday(&end, NULL);
    diff = 1000000 * (end.tv_sec - start.tv_sec) + end.tv_usec - start.tv_usec; // 實際的時間差
    printf("str_heapsort int:%ld us (equal %f sec)\n", diff, diff / 1000000.0);
    sum += diff / 1000000.0;
    fclose(intData);
    fclose(strData);
    printf("sum:%f\n",sum);
